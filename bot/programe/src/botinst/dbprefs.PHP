...
if (get_magic_quotes_gpc() && !isset($GLOBALS['socket_server'])) 
{
	if( is_array($_GET) )
...
*/ 
define("DEFAULTPREDICATEVALUE", "%USER%NAME%");
...
$rootdir = dirname(__FILE__) . "/../../aiml/";

if(!isset($GLOBALS['fc_config']['db']))
{
    require_once dirname(__FILE__) . '/../../../../inc/config.srv.php';
}

$DB_HOST  = $GLOBALS['fc_config']['db']['host'];
$DB_UNAME = $GLOBALS['fc_config']['db']['user'];
$DB_PWORD = $GLOBALS['fc_config']['db']['pass'];
$DB_DB    = $GLOBALS['fc_config']['db']['base'];
