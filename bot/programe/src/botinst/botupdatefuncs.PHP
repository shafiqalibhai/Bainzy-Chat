...
function do_bot_mysql_error($queryparam) {  // added by Veronica for MySQL tracing

 //  print "MySQL query: " . $queryparam . "<br>\n";
   if(mysql_error()) {
      print "<b>MySQL error no: " . mysql_errno() . " " . mysql_error() . "<BR>MySQL query: " . $queryparam . "<br></b>\n";
    //  print "<b>MySQL error no: " . mysql_errno() . " " . mysql_error() . "<BR></b>\n";
    //  if(mysql_errno() == 1226) die("Program stopped, can't continue !<br>Try to load aiml files one by one!");
   }
}
...
function deletebot()
{
    ...
    print "<b>Clearing Database Tables...<BR></b>\n";	

    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}bot";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }

    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}bots";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}conversationlog";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}dstore";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}gmcache";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}gossip";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}patterns";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}templates";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}thatindex";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
    $q="TRUNCATE TABLE {$GLOBALS['fc_config']['db']['pref']}thatstack";
    $e = mysql_query($q);
    do_bot_mysql_error($q);
    if ($e){
    }
...
function insertwordpattern($qadd)
{
    ...
    $qcode=mysql_query("insert into {$GLOBALS['fc_config']['db']['pref']}patterns(bot,id,word,ordera,parent,isend) values $qadd");
    do_bot_mysql_error("insert into {$GLOBALS['fc_config']['db']['pref']}patterns(bot,id,word,ordera,parent,isend) values $qadd");
	if ($qcode){
...
function insertmytemplate($idused,$template, $flushlast=false)
{
	...
	global $selectbot,$templatesinserted, $pattern, $topic, $that, $templBuf;
	...
	$pattern = addslashes($pattern);  // added by Veronica
		$templBuf[] = "($selectbot, $idused,'$template','$pattern','$that','$topic')";
		
		if( sizeof($templBuf) >= 200 )
		{
...
function learnallfiles($curbot)
{
    ...
    print "<b>Loading your AIML Directory...<BR></b>\n";

    global $rootdir,$learnfiles;
    $dir=opendir ($rootdir);
    while ($file = readdir($dir)) {
      if (strtoupper($file) != ".AIML")
      {
