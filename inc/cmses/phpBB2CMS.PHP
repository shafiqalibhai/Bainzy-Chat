...
//Geno Mod - added user_rank to getUserStmt
		$this->getUserStmt = new Statement("SELECT user_id as id, username as login, user_level, user_rank FROM {$GLOBALS['table_prefix']}users WHERE user_id=? AND user_active<>0 LIMIT 1");
		...
		if($userid && ($rs = $this->getUserStmt->process($userid)) && ($rec = $rs->next())) {
         ...
         // USERS MUST have a rank of 2 or they'll be banned!
              switch($rec['user_rank']) {
               ...
               case 2: $rec['user_level'] = ROLE_USER; break;
               case 3: $rec['roles'] = ROLE_MODERATOR; break;
               case 3: $rec['user_level'] = ROLE_MODERATOR; break;
